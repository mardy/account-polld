...
func (p *ImapPlugin) createNotifications(messages []*Message) []*plugins.PushMessage {
	...
	for _, msg := range messages {
		...
		if timestamp.Sub(msg.date) < timeDelta { // TODO: Remove this when the SINCE implementation is done as it causes problem with the limitation of downloading only 3 messages when these are too old
			...
			summary := sender
			body := fmt.Sprintf("%s\n%s", strings.TrimSpace(msg.subject), message[:int(math.Min(math.Max(float64(len(message)-1), 0), 400))]) // We do not need more than 400 characters (math.Max() to make sure we get no slice bounds out of range error due to the length being 0)
